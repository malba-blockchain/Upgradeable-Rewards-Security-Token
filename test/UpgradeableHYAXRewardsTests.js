...
describe("Testing Use Case #1: Constructor", function () {
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        const owerAddress = "0x0000000000000000000000000000000000000000"; // Replace with actual owner address
        const lockingTime = "1000";
        const upgradeableHYAXRewards = await ethers.deployContract("UpgradeableHYAXRewards");

        // Fixtures can return anything you consider useful for your tests
        return { upgradeableHYAXRewards, owner, addr1, addr2 };
    }

    it("Should deploy with correct initial parameters", async function () {
        const { upgradeableHYAXRewards, owner } = await loadFixture(deployUpgradeableHYAXRewardsFixture);

        // Replace 'getInitialParam1' and 'getInitialParam2' with actual public getter functions
        expect(await upgradeableHYAXRewards.owner()).to.equal(owerAddress);

    });

    it("Should have correct initial state", async function () {
        const { upgradeableHYAXRewards } = await loadFixture(deployUpgradeableHYAXRewardsFixture);

        // Replace with actual state variable names and values
        expect(await upgradeableHYAXRewards.lockingTime()).to.equal(lockingTime);
    });

    it("Should reject deployment with invalid parameters", async function () {
        const [owner] = await ethers.getSigners();

        const invalidLockingTime = "1000"; // Example of an invalid parameter

        await expect(
            ethers.deployContract("UpgradeableHYAXRewards", [invalidLockingTime])
        ).to.be.revertedWith("Invalid parameter error message"); // Replace with actual error message
    });
...
describe("Testing Use Case #2: Add Wallet to Whitelist", function () {
    ...
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        const initialParam1 = "Initial Value 1"; // Replace with actual initial parameters
        const initialParam2 = 1000; // Replace with actual initial parameters

        const upgradeableHYAXRewards = await ethers.deployContract("UpgradeableHYAXRewards", [initialParam1, initialParam2]);
...
describe("Testing Use Case #4: Fund Smart Contract", function () {
    ...
    it("Should successfully fund the contract with a specified amount and type", async function () {
        ...
        const fundingAmount = ethers.utils.parseUnits("1000", 18); // Replace with appropriate unit
    ...
    it("Should revert when funding with an invalid amount", async function () {
        ...
        const fundingType = "TeamRewards";
        const invalidAmount = ethers.utils.parseUnits("0", 18); // Example of invalid amount
    ...
    it("Should correctly allocate funds based on the specified funding type", async function () {
        ...
        const teamRewardsAmount = ethers.utils.parseUnits("1000", 18);
        const investorRewardsAmount = ethers.utils.parseUnits("500", 18);
    ...
    it("Should handle or reject excess funding appropriately", async function () {
        ...
        const fundingType = "TeamRewards";
        const excessiveAmount = ethers.utils.parseUnits("1000000", 18); // Example of excessive amount
...
describe("Testing Use Case #5: Withdraw Locked Tokens for Team Member", function () {
    ...
    it("Should handle partial withdrawals according to the schedule", async function () {
        ...
        const lockAmount = ethers.utils.parseUnits("1000", 18);
        const partialAmount = ethers.utils.parseUnits("200", 18);
        const lockPeriod = 30 * 24 * 60 * 60; // Lock period in seconds (e.g., 30 days)
        const unlockTime = (await ethers.provider.getBlock()).timestamp + lockPeriod;
...
describe("Testing Use Case #6: Withdraw Locked Tokens for Growth", function () {
    ...
    it("Should successfully withdraw growth tokens at the specified rate", async function () {
        ...
        const fundAmount = ethers.utils.parseUnits("10000", 18);
        const withdrawAmount = ethers.utils.parseUnits("500", 18); // Amount to withdraw
    ...
    it("Should revert if the withdrawal exceeds the allowed rate", async function () {
        ...
        const fundAmount = ethers.utils.parseUnits("10000", 18);
        const allowedAmount = ethers.utils.parseUnits("500", 18); // Allowed amount to withdraw based on the rate
        const exceedAmount = ethers.utils.parseUnits("600", 18); // Amount that exceeds the allowed rate
        const oneYear = 365 * 24 * 60 * 60; // One year in seconds
    ...
    it("Should revert if attempting to withdraw more than available tokens", async function () {
        ...
        const fundAmount = ethers.utils.parseUnits("10000", 18);
        const withdrawAmount = ethers.utils.parseUnits("12000", 18); // Amount to withdraw exceeds available balance
        const oneYear = 365 * 24 * 60 * 60; // One year in seconds
...
describe("Testing Use Case #7: Update Values of Token Rewards", function () {
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        // Assuming these functions exist to set the authorized accounts
        await upgradeableHYAXRewards.setAutoUpdater(addr1.address); // Add addr1 as an auto updater

        return { upgradeableHYAXRewards, owner, addr1, addr2, addr3 };
    }
    ...
    it("Should successfully update rewards for each wallet", async function () {
        ...
        const rewards = {
            [addr1.address]: ethers.utils.parseUnits("1000", 18),
            [addr2.address]: ethers.utils.parseUnits("2000", 18)
        };
    ...
    it("Should revert if non-authorized account tries to update rewards", async function () {
        ...
        const rewards = {
            [addr3.address]: ethers.utils.parseUnits("1000", 18)
        };
    ...
    it("Should revert if updating rewards with invalid data", async function () {
        ...
        const rewards = {
            [addr1.address]: ethers.utils.parseUnits("-1000", 18) // Invalid reward amount
        };
    ...
    it("Should handle large updates correctly", async function () {
        ...
        for (let i = 0; i < numberOfAddresses; i++) {
            ...
            rewards[testAddress] = ethers.utils.parseUnits((i + 1).toString(), 18);
        }
...
describe("Testing Use Case #8: Withdraw Token Rewards", function () {
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        // Set whitelist status and rewards for test addresses
        await upgradeableHYAXRewards.setWhitelist(addr1.address, true);
        await upgradeableHYAXRewards.setRewards(addr1.address, ethers.utils.parseUnits("1000", 18));
    ...
    it("Should successfully withdraw token rewards for a whitelisted user", async function () {
        ...
        await expect(upgradeableHYAXRewards.connect(addr1).withdrawTokenRewards())
            ...
            .withArgs(addr1.address, ethers.utils.parseUnits("1000", 18));
        ...
        expect(remainingRewards).to.equal(ethers.utils.parseUnits("0", 18));
    });
    ...
    it("Should not affect contract state when withdrawing zero rewards", async function () {
        ...
        await upgradeableHYAXRewards.setRewards(addr1.address, ethers.utils.parseUnits("0", 18));
...
describe("Testing Use Case #9: Withdraw Tokens to Burn", function () {
    ...
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        // Fund the contract with tokens for testing
        const tokenAmount = ethers.utils.parseUnits("10000", 18);
        await upgradeableHYAXRewards.fundContract(tokenAmount);
    ...
    it("Should successfully withdraw tokens to burn", async function () {
        ...
        expect(contractBalance).to.equal(ethers.utils.parseUnits("5000", 18)); // Assuming initial balance was 10000
    });
    ...
    it("Should revert when attempting to burn more tokens than available", async function () {
        ...
        const excessiveBurnAmount = ethers.utils.parseUnits("20000", 18);
    ...
    it("Should accurately record burned tokens", async function () {
        ...
        const contractBalance = await upgradeableHYAXRewards.balanceOf(upgradeableHYAXRewards.address);
        expect(contractBalance).to.equal(ethers.utils.parseUnits("8000", 18)); // Assuming initial balance was 10000
    });
    ...
    it("Should revert if burning unauthorized tokens", async function () {
        ...
        const unauthorizedBurnAmount = ethers.utils.parseUnits("500", 18);
        await expect(
...
describe("Testing External Use Case #11: Calculate Token Rewards Earned Weekly", function () {
    async function deployUpgradeableHYAXRewardsFixture() {
        ...
        // Set up any initial data if required
    ...
    it("Should calculate and update rewards correctly based on weekly data", async function () {
        ...
        const initialRewards = [
            { account: owner.address, reward: ethers.utils.parseUnits("100", 18) },
            { account: addr1.address, reward: ethers.utils.parseUnits("50", 18) },
            { account: addr2.address, reward: ethers.utils.parseUnits("75", 18) }
        ];
    ...
    it("Should handle large datasets efficiently and accurately", async function () {
        ...
        for (let i = 0; i < numAccounts; i++) {
            ...
            const reward = ethers.utils.parseUnits((Math.random() * 100).toFixed(2), 18);
    ...
    it("Should handle edge cases such as zero or very high reward values", async function () {
        ...
        const edgeCases = [
            ...
            { account: addr1.address, reward: ethers.utils.parseUnits("1000000", 18) } // Very high reward
        ];
